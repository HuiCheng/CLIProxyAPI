...
- **缺少鉴权。** 不加 `--api` 再 POST。期望 HTTP `401`。
- **证明。** 保留 models 与 generate 两份响应。

## 注意事项

- 路径里的模型名是客户端别名，不是上游 `mock-model`。
- 代理会翻译成 openai-compat `/chat/completions`。
- `/v1beta/interactions` 是另一入口。本文件基线不替代它。若要证明 interactions，另存 `gemini-generate/interactions.txt` 并在 notes 标明。
